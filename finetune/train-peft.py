...
class ModelArguments:
    ...
    total_step: int = field(default=10000, metadata={"help": "total training steps"})
    tinit: int = field(default=1000, metadata={"help": "warmup steps before rank adaptation starts"})
    tfinal: int = field(default=8000, metadata={"help": "end step of adaptation"})
    deltaT: int = field(default=100, metadata={"help": "frequency of rank updates"})
...
def compute_metrics(eval_pred):
    ...
    results = calculate_metric_with_sklearn(predictions, labels)
    valid_mask = labels != -100
    results["roc_auc"] = sklearn.metrics.roc_auc_score(labels[valid_mask], probs[valid_mask, 1])
    results["pr_auc"] = sklearn.metrics.average_precision_score(labels[valid_mask], probs[valid_mask, 1])
    return results
...
def train():
    ...
    config.pad_token_id = tokenizer.pad_token_id  # IMPORTANT: fixes the BertEmbeddings error
    ...
    if peft_method == "qlora":
        bnb_config = transformers.BitsAndBytesConfig(
            ...
            # modules_to_not_convert=["pooler", "classifier"], # doesn't work
        )
