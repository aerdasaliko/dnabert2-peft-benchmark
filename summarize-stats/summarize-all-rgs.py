...
LOGS_DIR = "../all-logs/logs-regression"
...
GPU_OVERLAYS_DIR = os.path.join(LOGS_DIR, "final_timeseries")
...
METRIC_ORDER = ["MSE", "RMSE", "MAE", "MAPE", "$R^2$", "Pearson", "Spearman"]
...
# A F1 Score
axs[0, 0].bar(merged["method_pretty"], merged["eval_r2"], color=color_for_methods(methods))
axs[0, 0].set_title("A. R^2 Score")
...
# C Pareto
axs[1, 0].scatter(merged["runtime_mean"], merged["eval_r2"],
                  c=[METHOD_COLORS.get(m, "#333333") for m in methods])
for i, m in enumerate(methods):
    axs[1, 0].annotate(m, (merged["runtime_mean"].iloc[i], merged["eval_r2"].iloc[i]))
axs[1, 0].set_title("C. F1 vs Runtime (Pareto View)")
axs[1, 0].set_xlabel("Runtime (s)")
axs[1, 0].set_ylabel("F1")
axs[1, 0].grid(alpha=0.3)

points = merged[["runtime_mean", "eval_r2"]].values
...
# D Memory Usage vs R^2
axs[1, 1].scatter(merged["memu_peak_mib"], merged["eval_r2"],
                  c=[METHOD_COLORS.get(m, "#333333") for m in methods])
for i, m in enumerate(methods):
    axs[1, 1].annotate(m, (merged["memu_peak_mib"].iloc[i], merged["eval_r2"].iloc[i]))
...
norm["overall_score"] = 0.4 * norm["eval_r2"] + 0.3 * norm["runtime_score"] + 0.3 * norm["resource_score"]
